	...
	<div class="container">
		...
		<div class="row">
			...
			<div id="main" class="col-md-12">
				...
				<div class="pull-left">
					<?php if(isset($k['kategori_nama'])): ?>
						<h4>Kategori Produk : <?php echo $k['kategori_nama']; ?></h4>
					<?php else: ?>
						<h4>Kategori Produk tidak tersedia</h4>
					<?php endif; ?>
				</div>
				...
				<div id="store">
					...
					if (isset($_GET['urutan']) && $_GET['urutan'] == "harga") {
						$data = mysqli_query($koneksi, "SELECT * FROM produk, kategori WHERE kategori_id='$id' AND kategori_id=produk_kategori ORDER BY produk_harga ASC LIMIT $mulai, $halaman");
					} else {
						$data = mysqli_query($koneksi, "SELECT * FROM produk, kategori WHERE kategori_id='$id' AND kategori_id=produk_kategori ORDER BY produk_id DESC LIMIT $mulai, $halaman");
					}
					$no = $mulai + 1;
						...
						<ul class="store-pages">
							...
							<?php for ($i=1; $i<=$pages ; $i++){ ?>
								...
								<?php }else{ ?>
									...
									if(isset($_GET['urutan']) && $_GET['urutan'] == "harga"){
										?>
										<li><a href="?id=<?php echo $_GET['id']; ?>&halaman=<?php echo $i; ?>&urutan=harga"><?php echo $i; ?></a></li>
										<?php 
									}else{
										?>
										<li><a href="?id=<?php echo $_GET['id']; ?>&halaman=<?php echo $i; ?>"><?php echo $i; ?></a></li>
										<?php
									}
									 ?>
									<?php } ?>
